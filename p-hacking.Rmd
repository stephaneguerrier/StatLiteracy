...
.pull-left[
.smaller[.hi-pink[Statistics] is a science that uses mathematics and computer sciences to deal with the collection, analysis, interpretation, and presentation of masses of numerical data. Unformally, it is the .pink[science of learning from data]. 🧐]
...
.pull-right[
.smaller[.hi-purple[Statistics] is a crucial part of our life. However, .purple[statistical methods are often misused consciously or not] (but how? 🤔). This can lead to contradictory studies and conclusions (as seen during the current COVID-19 pandemic).]
...
- .smallest[The ability to .pink[understand and reason with data], or arguments that use data.]
...
👋 .smallest[If you want to know more about this comics have a look [here](https://www.explainxkcd.com/wiki/index.php/552:_Correlation).]
]

.smallest[.hi-purple[Statistical literacy is now becoming an essential skill]. It is, for exeample, needed by .purple[anyone who tries to learn new information from data]. .turquoise[Scientists also need to develop statistical literacy so that they can both produce rigorous and reproducible research.]]
...
👋 .smaller[If you want to know more about this comics have a look [here](https://www.explainxkcd.com/wiki/index.php/795:_Conditional_Risk).]

---

# How can statistics be useful?

As mentionned previously, Statistics is the .pink[science of learning from data]. It can be used (among other) to:
1. .purple[Vizualise data] (e.g. propagation of COVID-19 in different countries).
2. .purple[Understand and interpret data] (e.g. main causes of cancer?). 
3. .purple[Make prediction] (e.g. predicting unemployement or risk indices).
4. .hi-pink[Assess the validity of an hypothesis].

In this class, we will focus on the last point:
''.pink[*how to assess the validity of an hypothesis using statistics?*]''. More precisely, we will discuss how statistical methods can be used to evaluate claims (hypothesis) such as:
...
- Statistics methods are based on several fundamental concepts, the most central of which is that of considering the information available (in the form of data) as resulting from a .hi-pink[random process]. 🙉
...
- Statistics allows to determine the .hi-pink[decision-making risks] (in the form of probabilities) associated with learning (decisions) based on the analysis of data made on samples. .slate[This allows to evaluate the validity of a claim] (such as the ones we discussed previously). 
- .purple[Understanding and knowing how to interpret statistical analyses is therefore becoming an increasingly vital skill].
- In some sense, .hi-pink[statistics plays a crucial part in our lifes] as new discoveries (obtained using statistical methods) directly impact us<sup>.smallest[👋]</sup>.
- However, understanding statistical results is not intuitive. There exists many pitfall such as .purple[p-hacking] or .purple[HARKing] (e.g. why are there so many contraditory studies? Is Hydroxychloroquine working or not?).
...
Since the beginning of 2020, various research teams have suggested that (hydroxy)chloroquine combined with other drugs (e.g. azithromycin) was an .hi-pink[effective treatment] for the coronavirus disease 2019. Since then there has been .hi-purple[many many many contradictory publications] (and some [retractions](https://en.wikipedia.org/wiki/The_Lancet#Retracted_papers_and_scientific_controversies)!) and the efficacy of these drugs remains unclear.

As summarized on [Wikipedia](https://en.wikipedia.org/wiki/Hydroxychloroquine#COVID-19) dedicated to the hydroxychloroquine (and citing various scientific sources): .turquoise["*There is no strong scientific evidence to support the use of hydroxychloroquine for preventing or treating coronavirus disease 2019 (COVID‑19)*"].

.hi-pink[So what does this mean?] 🤔 

Does it mean that hydroxychloroquine is not effective for preventing or treating COVID‑19? That it is effective? Actually, neither...  🤔 
...
- The Swiss population of voter is composed of 1200 individuals (576 for the project, 624 against).
- We can perfectly sample the population (everyone is available, no double sampling, the sampling is random).
...
- .smaller[These intervals corresponds to a .pink[range of values that are likely to include a population value with a certain level of confidence]. The level of confidence is a probability expressed as a %.] 
...
.center[![Gapminder](GIF/sample60.gif)]

---

# What if I want to be really sure? 99%?

.center[![Gapminder](GIF/sample99.gif)]

---

# A bit more? How about 99.99%?

.center[![Gapminder](GIF/sample9999.gif)]

---

# What if I want to be certain? 100%? 😳

.center[![Gapminder](GIF/sample100.gif)]
...
- The only certainty one can have is that, for example, a proportion will always be included in the interval from 0% to 100%. .hi-purple[However, this is not informative nor useful] 🧐 and it does not even depend on the data.
- There exist a tradeoff between .hi-pink[risk] as measured by  1 - $\alpha$ (typically 95%) the confidence level, and the .hi-pink[precision of the conclusion] as measured, for example, by the confidence interval length.
- Moreover, the larger the sample size, the more precise the conclusion, for the same confidence level.
- Therefore, .purple[every decision based on statistical methods has a risk and how much risk is acceptable depends on the context] (safety in airplanes vs which soft drink tastes better).
...
- .smaller[However, p-values have been misused many times because understanding what they mean is not intuitive.]
...
- .smaller[A p-value is associated to a (couple of) .purple[hypotheses] about the phenomenon under investigations. For example, for the voting proposal, one hypothesis is that the majority is against the proposal, and the other one is that there is no majority (the YES and NO are both at 50%)]
- .smaller[Each hypothesis .purple[exclude each other], so that one can .purple[exclude one in favor of the other] using the data.]
- .smaller[The .purple[null hypothesis] is the one that one will never be able to prove because the data is random, i.e. the most precise. In the example it is the hypothesis that the votes are 50%-50%.]
...
- .smaller[Informally, .hi-pink[it can be understood as a measure of plausibility of the null hypothesis given the data]. The smaller the p-value the greater the incompatibility of the null hypothesis with the data.]

---

# Testing

- .smaller[When this happens, one says that the test based on the null and alternative hypothesis is .pink[significant] or that the null hypothesis is rejected in favour of the alternative.]
- .smaller[If this does not happen, with the available data, then .pink[nothing] can be concluded.]
- .smaller[With a sample of data, the obtained p-value (associated to a couple of hypothesis) summarizes somehow the .pink[incompatibility between the data and the model] (random process) constructed under the set of assumptions.]
- .smaller[The (sample) p-value is traditionnally compared to a .pink[threshold value] that sets the (subjective) risk level of deciding in favor of the incompatibility.]
- .smaller[The risk level is called the .pink[significance level] and is a small value, usually 5%.]
...
- .smaller[A p-value is a random variable, since its value depends on the data. Likewise, confidence intervals are random, since their bounds depend on the data.]
- .smaller[Hence, by chance, it might happen that while the null hypothesis cannot be rejected (supposing it is true), that the p-value is smaller than the set threshold. With the later chosen as 5%, then, on average, .pink[the (sample) p-value is below 5% once in twenty times!]]
- .smaller[Hence performing .pink[multiple tests], with the same or different data, automatically leads to .pink[significant results, when actually there are none!]]
- .smaller[Confidence intervals and p-values can be used for many types of models, such as regression, correlation, and so on.]

+ DIFF BETWEEN TEST AND CI (I would not...)


---

# Why are there contraditory studies?
...
.smaller[Are republicans are good or bad for the American Economy? .hi-purple[Both!!] (see [link](https://projects.fivethirtyeight.com/p-hacking/))]
...
- .smaller[Any data analysis made on a sample of data is subject to randomness.]
- .smaller[Understanding the random processes underlying any data analysis, including computing statistics such as confidence intervals and p-values, requires basic knowledge in probability and statistics, or .pink[statistical litterarcy].]
- .smaller[With data, there is never 100% certainty, but rather a .pink[conclusion associated to a (estimated) risk] that the conlcusion might be wrong.] 
- .smaller[One has to live with this fact, and any scientific activity that might conclude with 100% certainty on the basis of data, is simply a fallacy.]
- .smaller[However, properly controlling for the statistical risk and properly understanting the conclusions that can be drawn from a data analysis, can really bring new knowledge, especially in all the sciences that are based on observations.]
- .smaller[Maybe add something (a link) about good practice in data analysis as proposed in many places...]
