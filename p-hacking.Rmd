...
As mentionned previously, statistics is the .pink[science of learning from data]. It can be used (among others) to:
...
- In some sense, .hi-pink[statistics plays a crucial part in our life] as new discoveries (obtained using statistical methods) directly impact us<sup>.smallest[👋]</sup>.
...
- .smaller[These intervals correspond to a .pink[range of values that are likely to include the population value with a certain level of confidence]. The level of confidence is a probability expressed as a %.] 
...
- .smaller[Each hypothesis .purple[exclude each other], so that onse can .purple[exclude one in favor of the other] using the data.]
- .smaller[The .purple[null hypothesis] is the one that one will never be able to prove because the data is random, i.e. the most precise.he hypothesis that the votes are 50%-50%.]
- .smaller[The .purple[alternative hypothesis] is the one that offers more choice of values and hence hass a chance to be favored with respect to the null hypothesis (e.g. proportion of NO is smaller than 50%).]
- .smaller[Informally, .hi-pink[it can be understood as a measure of plausibility of the null hypothesis given the data]. The smaller the p-value the greater the incompa,tibility of the null hypothesis with the data.]
