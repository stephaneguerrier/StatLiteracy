...
- .smaller[The (sample) p-value is traditionally compared to a .pink[threshold value] that sets the (subjective) risk level of deciding in favor of the incompatibility.]
- .smaller[The risk level is called the .pink[significance level] and is a small value, usually 5%, but again this depends of the context.]
...
# What to remeber about p-values
...
- .smaller[Hence, by chance, it might happen that while the null hypothesis cannot be rejected (supposing it is true), that the p-value is smaller than the set threshold. With the later chosen as 5%, then, on average, the (sample) p-value is below 5% .purple[once in twenty times!]]
...
- .smaller[Confidence intervals and p-values can be used for many types of models, such as regression, correlation, and so on. Therefore, .purple[they can be use to assess the validity of most (scientific) hypothesis] (as discuss in your statistics classes! 🤓).]
...
- Understanding the random processes underlying any data analysis, including computing statistics such as confidence intervals and p-values, requires basic knowledge in probability and statistics, or .pink[statistical literacy].
...
- However, properly controlling for the statistical risk and properly understating the conclusions that can be drawn from a data analysis, can really bring new knowledge, especially in all the sciences that are based on observations.

---

# Thank you very much for your attention

.center[Slides are available [here]()]

.center[All material is available on [GitHub](https://github.com/stephaneguerrier/StatLiteracy)]
